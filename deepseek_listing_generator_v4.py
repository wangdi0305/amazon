# -*- coding: utf-8 -*-
"""
汽配零件 Listing 自动生成工具 (浏览器版 v4 - 最终优化版)
...
def parse_deepseek_response(text, oe_number):
    """
    智能解析 DeepSeek 的回复（空格分隔表格 + 段落内容）。

    DeepSeek 输出结构：
    1. 表格表头（空格分隔）: A G AD AE AF AG AH AI AJ AK AL AM AN
    2. 表格数据（空格分隔）：OE号, OE号(重复), Title...
    3. 详细段落（带关键词）：Vehicle Fitment: OEM Cross Reference: 等
    """
...
def main():
    print("=" * 60)
    print("  汽配 Listing 自动生成工具 v4 (最终优化版)")
    ...
    try:
        ...
        for i, (row, oe) in enumerate(work):
            ...
            # 调试: 保存原始回复
            if DEBUG_MODE:
                debug_file = f"C:\\Users\\Administrator\\Desktop\\debug_v4_row{row}.txt"
