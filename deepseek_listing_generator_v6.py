# -*- coding: utf-8 -*-
"""
汽配零件 Listing 自动生成工具 (浏览器版 v6 - 修正版)

关键修复：
1. 修正列号映射问题
2. 确保数据正确写入对应的 Excel 列
"""
import openpyxl
import shutil
import time
...
def parse_deepseek_response(text, oe_number):
    ...
    precise_section = extract_section(text, precise_patterns,
                                      ['Plug', 'Durable', '1-Year', '写作', 'Factory-Grade'])
    if precise_section:
    ...
    plug_section = extract_section(text, ['Plug & Play Installation:', 'Plug Play Installation:'],
                                   ['Durable', '1-Year', '写作', 'Factory-Grade'])
    if plug_section:
        result['bullet4'] = clean_text(plug_section)

    # AI 列: Factory-Grade Quality: + 1-Year Warranty: -> 卖点5
    # 注意：DeepSeek 输出中 "Factory-Grade Quality:" 在 "1-Year Warranty:" 之前
    factory_section = extract_section(text, ['Factory-Grade Quality:', 'Factory-Grade:'],
                                      ['1-Year', '写作', '关键词'])
    if factory_section:
        result['bullet5'] = clean_text(factory_section)
    ...
    for row, data in results.items():
        ...
        for field, col in COL_MAP.items():
            ...
            if val:
                ...
                print(f"    {field:15} -> 列{col} ({len(val)} 字符)")
...
def main():
    print("=" * 60)
    print("  汽配 Listing 自动生成工具 v6 (修正版)")
    ...
    try:
        ...
        for i, (row, oe) in enumerate(work):
            ...
            # 调试: 保存原始回复
            if DEBUG_MODE:
                debug_file = f"C:\\Users\\Administrator\\Desktop\\debug_v6_row{row}.txt"
